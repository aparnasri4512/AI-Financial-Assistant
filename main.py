...
print(portfolio) #execute at beginning to create portfolio file

#functions for each intent
def save_portfolio():
...
def show_portfolio():
    print("Your protfolio:")
    for ticker in portfolio.key():
...
def plot_chart():
    ...
    start = dt.datetime.strptime(starting_string, "%d/%m/%Y") #coverting string to date
    end = dt.datetime.now()
...
mappings = {
    ...
    'remover_portfolio': remove_portfolio,
...
assistant = GenericAssistant('intents.json', intent_methods=mappings, "financial_assistant_model")
